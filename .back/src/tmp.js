...
export default class Theme extends Control {
    constructor(options = {}) {
        ...
        this.saveLocal = options.saveLocal || true; //TODO
        ...
        this.content.className = 'taskpane theme';
    ...
    styleSet_() { //add style from this.layer.def.layer.style to this.element UI
        ...
        for (const [i, item] of this.content.querySelectorAll('.item').entries()) { // style item checked or not
    ...
    styleApply_() { //apply style from UI to def.layer.style and through makeStyle(def.layer.style) apply to layer
        const
            ...
            s = sName => this.content.querySelector(`.${sName} .check`);
        for (const i of this.content.querySelectorAll('.item')) {
            const is = {};
    ...
    setActive(b) {
        this.content.style.display = b ? 'flex' : 'none';
        if (this.content.innerHTML === '') this.theme_();
