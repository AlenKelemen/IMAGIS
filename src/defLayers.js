...
export default class DefLayers {
  constructor(options = {}) {
   ...
   /*  
    ...
    this.status.then(r => console.log('status:',r)) */
  ...
  removeTileLayers() {
    // remove 'osm', 'bing', 'wms' layers defined in def from map
    for (const [i, l] of this.getTileLayers().entries()) {
      this.map.removeLayer(
        this.map
          .getLayers()
  ...
  removeVectorLayers() {
    // remove 'geojson' layers defined in def from map
    for (const [i, l] of this.getVectorLayers().entries()) {
      this.map.removeLayer(
        this.map
          .getLayers()
  ...
  setStyle() {
    // set style from def to named layers
    if (l.style) layer.setStyle(makeStyle(l.style));
  }
  setDef(def) {
    //change def
