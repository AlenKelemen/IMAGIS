...
export default class Theme extends Control {
  constructor(options = {}) {
    ...
    const evtFunction = evt => {
      if (
        ...
        this.content.element.appendChild(this.header);
      }
  ...
  styleSet_() {
    ...
    for (const [i, item] of this.content.element
      ...
      if (style[i].resolution)
        item.querySelector(".resolution .resolution").value = style[
          i
        ].resolution.join();
      if (style[i].icon && style[i].icon.src)
        item.querySelector(".icon .src").value = style[i].icon.src;
      if (style[i].icon && style[i].icon.scale)
        item.querySelector(".icon .scale").value = style[i].icon.scale;
      if (style[i].icon && style[i].icon.anchor)
        item.querySelector(".icon .anchor").value = style[i].icon.anchor.join();
      if (style[i].regularShape && style[i].regularShape.points)
        item.querySelector(".regularShape .points").value =
          style[i].regularShape.points;
      if (style[i].regularShape && style[i].regularShape.radius)
        item.querySelector(".regularShape .radius").value =
          style[i].regularShape.radius;
      if (
        style[i].regularShape &&
        style[i].regularShape.fill &&
        style[i].regularShape.fill.color
      )
        item.querySelector(".regularShape .fill.color").style.backgroundColor =
          style[i].regularShape.fill.color;
      if (
        style[i].regularShape &&
        style[i].regularShape.stroke &&
        style[i].regularShape.stroke.color
      )
        item.querySelector(
          ".regularShape .stroke.color"
        ).style.backgroundColor = style[i].regularShape.stroke.color;
      if (
        style[i].regularShape &&
        style[i].regularShape.stroke &&
        style[i].regularShape.stroke.width
      )
        item.querySelector(".regularShape .stroke.width").value =
          style[i].regularShape.stroke.width;
      if (style[i].stroke && style[i].stroke.color)
        item.querySelector(".stroke .color").style.backgroundColor =
          style[i].stroke.color;
      if (style[i].stroke && style[i].stroke.width)
        item.querySelector(".stroke .width").value = style[i].stroke.width;
      if (style[i].fill && style[i].fill.color)
        item.querySelector(".fill .color").style.backgroundColor =
          style[i].fill.color;
      ...
      if (style[i].text && style[i].text.placement)
        item.querySelector(".text .placement").value = style[i].text.placement;
      if (style[i].text && style[i].text.textBaseline)
        item.querySelector(".text .textBaseline").value =
          style[i].text.textBaseline;
      if (style[i].text && style[i].text.scale)
        item.querySelector(".text .scale").value = style[i].text.scale;
      if (style[i].text && style[i].text.font)
        item.querySelector(".text .font").value = style[i].text.font;

      if (style[i].filter && style[i].filter.property) {
        item.querySelector(".filter .properties").value =
          style[i].filter.property;
        item
          .querySelector(".filter .properties")
          .dispatchEvent(new Event("change"));
      }
      if (style[i].filter && style[i].filter.operator)
        item.querySelector(".filter .operators").value =
          style[i].filter.operator;
      if (style[i].filter && style[i].filter.value) {
        if (
          item.querySelector(
            `.filter .constrains select[value="${style[i].filter.value}"]`
          )
        )
          item.querySelector(".filter .constrains").value =
            style[i].filter.value;
  ...
  styleAdd_() {
    ...
    this.content.element.appendChild(this.wrapper); //item wrapper
    ...
    for (const j of this.wrapper.querySelectorAll(".caret")) {
      j.addEventListener("click", evt => {
        ...
        j.parentElement.querySelector(".nested").style.display =
          j.parentElement.querySelector(".nested").style.display === "block"
            ? "none"
            : "block";
    ...
    //chech style item
    for (const j of this.wrapper.querySelectorAll(
  ...
  styleApply_() {
    ...
    const ns = [], //new style
      ...
      s = sName => this.content.element.querySelector(`.${sName} .check`);
    for (const i of this.content.element.querySelectorAll(".item")) {
      const is = {};
      for (const e of i.querySelectorAll(".fa-check-square")) {
        ...
        if (e.closest("ul").className === "regularShape") {
          is.regularShape = {};
          is.regularShape.points = e
            .closest("ul")
            .querySelector(".points").value;
          is.regularShape.radius = e
            .closest("ul")
            .querySelector(".radius").value;
          is.regularShape.fill = {};
          is.regularShape.fill.color = e
            .closest("ul")
            .querySelector(".fill.color").style.backgroundColor;
          is.regularShape.stroke = {};
          is.regularShape.stroke.color = e
            .closest("ul")
            .querySelector(".stroke.color").style.backgroundColor;
          is.regularShape.stroke.width = e
            .closest("ul")
            .querySelector(".stroke.width").value;
        }
        if (e.closest("ul").className === "stroke") {
          is.stroke = {};
          is.stroke.color = e
            .closest("ul")
            .querySelector(".color").style.backgroundColor;
        ...
        if (e.closest("ul").className === "text") {
          is.text = {};
          if (e.closest("ul").querySelector(".text").value)
            is.text.text = "*" + e.closest("ul").querySelector(".text").value;
          else
            is.text.text = e.closest("ul").querySelector(".properties").value;
          is.text.placement = e.closest("ul").querySelector(".placement").value;
          is.text.textBaseline = e
            .closest("ul")
            .querySelector(".textBaseline").value;
        ...
        if (e.closest("ul").className === "filter") {
          is.filter = {};
          is.filter.property = e
            .closest("ul")
            .querySelector(".properties").value;
          is.filter.operator = e
            .closest("ul")
            .querySelector(".operators").value;
          if (e.closest("ul").querySelector(".value").value)
            is.filter.value = e.closest("ul").querySelector(".value").value;
          else
            is.filter.value = e
              .closest("ul")
              .querySelector(".constrains").value;
        }
      }
      ns.push(is);
    }
  ...
  fillProperties_(itemElement) {
    ...
    for (const e of itemElement.querySelectorAll(".properties")) {
      ...
      for (const p of this.layer.getSource().get("def").schema.properties)
        e.add(new Option(p.Label, p.Name));
    }
  }
  ...
  setLayer(layer) {
    if (!layer) {
      ...
      this.header
        .querySelector(".add")
        .addEventListener("click", evt => this.styleAdd_());
      this.header
        .querySelector(".apply")
        .addEventListener("click", evt => this.styleApply_());
